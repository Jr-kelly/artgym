"""Gate and run the bounded head/full-actor action-imitation comparison."""
import argparse
import hashlib
import json
...
def main():
    ...
    try:
        ...
        for name,count,updates,lr in stages:
            out=run/name
            command=[sys.executable,'-m','scripts.train_wuji_actor_imitation',
            ...
            assert all(report[k] for k in ['teacher_unchanged','student_encoder_unchanged',
