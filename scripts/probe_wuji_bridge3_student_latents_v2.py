...
def main():
    ...
    def make_player(cfg, checkpoint):
        ...
        assert network.separate and network.base_obs_dim == 132 and network.critic_policy_contact_dim == 5
        metadata['counterfactual_critic_state_checks'] = 0
        ...
        def observed(obs, is_deterministic=False, **kwargs):
            ...
            try:
                with torch.no_grad():
                    for label in labels:
                        ...
                        if label != args.driver and player.is_rnn:
                            assert all(torch.equal(a, b) for a, b in zip(player.states[2:], chosen_states[2:])), 'Critic counterfactual changed under identical raw input/RNN'
                            metadata['counterfactual_critic_state_checks'] += env.num_envs
            ...
            row = dict(raw_observation=obs, active=env.eval_active_mask, driver_action=outputs[args.driver],
            ...
            saved = {key: value.detach().cpu().numpy().copy() for key, value in row.items()}
            if player.is_rnn:
                actor_bytes = b''.join(v.detach().cpu().numpy().tobytes() for v in chosen_states[:2])
                saved['actor_rnn_sha256'] = np.frombuffer(hashlib.sha256(actor_bytes).digest(), dtype=np.uint8).copy()
            frames.append(saved)
    ...
    report.update(probe_driver=args.driver, probe_source_sha256=metadata['source_sha256'],
                  probe_driver_only=args.driver_only, diagnostic_version=2,
