...
def main():
    ...
    directory = ROOT / 'runs/wuji-goal/diagnostics/bridge3-encoder-cp100-diagnostic-v2'
    ...
    def job(driver, cp, rows, suffix, driver_only=False):
        ...
        return dict(name='bridge3-encoder-v2-%s-%s' % (driver, suffix),
                    module='scripts.probe_wuji_bridge3_student_latents_v2',
    ...
    for driver in ['teacher', 'plain', 'controller']:
        ...
        base = folder / ('bridge3-encoder-v2-%s-gate-cp25-driver-only' % driver)
        observed = folder / ('bridge3-encoder-v2-%s-gate-cp25-observed' % driver)
        matched, critic_differences = {}, {}
        for name in ['trace.npz', 'encoder_error_trace.npz']:
            a, b = np.load(base / name), np.load(observed / name)
            for key in a.files:
                assert key in b.files, (driver, name, key)
                u, v = a[key], b[key]
                if key == 'raw_observation':
                    assert u.shape[-1] == v.shape[-1] == 138
                    critic_differences['contact_aux_changed_values'] = int(np.count_nonzero(u[:, :, 132:137] != v[:, :, 132:137]))
                    u = np.concatenate([u[:, :, :132], u[:, :, 137:]], -1)
                    v = np.concatenate([v[:, :, :132], v[:, :, 137:]], -1)
                elif key == 'driver_rnn_sums':
                    critic_differences['critic_rnn_max_delta'] = float(np.max(abs(u[:, :, 2:]-v[:, :, 2:])))
                    u, v = u[:, :, :2], v[:, :, :2]
                assert np.array_equal(u, v, equal_nan=True), (driver, name, key)
                matched[name + ':' + key] = list(u.shape)
        comparisons.append(dict(driver=driver, actor_and_physics_exact_match=True, matched_shapes=matched, critic_differences=critic_differences))
    atomic_json(directory / 'noninterference.json', dict(status='passed', finished=now(), comparisons=comparisons,
                scope='Same3env600steps: physical traces, actions, actor-input and full actor-RNN hashes exact. Five critic-only contact inputs may vary across simulator processes and are reported; within each same-input counterfactual, critic-RNN outputs are exact. No assertion of task success.'))
