...
def main():
    ...
    def free(v,geometry=g):
        ...
        return geometry.minimum_gap(q,relative,slider)>=.0041 and all(s['gap_lower_bound_m']>=0 for s in geometry.self_gaps(q,'thumb'))
    ...
    trees=[dict(nodes=[start],parents=[-1]),dict(nodes=[goal],parents=[-1])];active=0;iterations=0
    ...
    while path is None and iterations<a.iterations and time.monotonic()-started<a.seconds and sum(len(t['nodes']) for t in trees)<1600:
        ...
        active=1-active
    report.update(iterations=iterations,collision_checks=checks,search_seconds=time.monotonic()-started,raw_path=None if path is None else [v.tolist() for v in path])
    ...
    plan=json.loads(a.prefix.read_text());stages=[dict(name='thumb_slider_start_hold',kind='hold',seconds=1.,required_initial_hand_command=cmd.tolist(),require_contacts=[1,2,3,4],require_no_contact=0,require_thumb_gap_m=.0041)];previous=cmd[16:]
    for j,v in enumerate(path[1:]):
        seconds=float(np.ceil(max(1.,1.875*np.max(abs(v-previous))/1.2)*30)/30)
        stages.append(dict(name='thumb_slider_free_'+str(j+1),kind='move',moving_indices=list(range(16,20)),target=v.tolist(),seconds=seconds,require_thumb_gap_m=.0041));previous=v
