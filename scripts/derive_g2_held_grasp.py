...
def main():
    ...
    i=int(np.flatnonzero(t['phase']=='lift')[-1]);physics=json.loads((a.trial/'physics.json').read_text())
    ...
    data.update(wrist_in_knife=relative.tolist(),touch_q=t['q'][i].tolist(),
        ...
        validation='Actual lift-end geometry for motor planning only. Original tabletop grasp plan remains unchanged; never load as simulator state.')
