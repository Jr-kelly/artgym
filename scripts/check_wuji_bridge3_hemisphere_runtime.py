"""Validate the bridge3 hemisphere task on actual training resets and physics.
...
def main():
    ...
    manifest_path=root/'runs/wuji-goal/bridge3-dataset-manifest.json'
    manifest=json.loads(manifest_path.read_text())
    assert manifest['status']=='frozen' and manifest['train_count']==3 and manifest['test_count']==1
    ...
    cfg=configuration('wuji_acquisition_bridge3_hemisphere',80,
        ['object='+dataset,'hand=wuji_paper_official_actuator','test=False','object.reward.GoalDistance2=5.0'],
        train='wujiAcquisitionSAPG',seed=20261045)
    (args.output/'config.yaml').write_text(OmegaConf.to_yaml(cfg,resolve=True))
    checks=dict(sampled_rows=[0]*3,transitions=0,clock_switches=0,action_mapping_calls=0)
    ...
    try:
        ...
        sources=['scripts/check_wuji_bridge3_hemisphere_runtime.py','scripts/wuji_quaternion_hemisphere.py','scripts/wuji_knife_frame.py','isaacgymenvs/tasks/wuji_bridge3_hemisphere.py','isaacgymenvs/cfg/task/wuji_acquisition_bridge3_hemisphere.yaml','isaacgymenvs/tasks/artmanip.py',
            'isaacgymenvs/tasks/wuji_acquisition.py','isaacgymenvs/tasks/wuji_timed_acquisition.py',
            'isaacgymenvs/cfg/object/'+dataset+'.yaml','runs/wuji-goal/bridge3-dataset-manifest.json']
        sources += ['isaacgymenvs/tasks/wuji_bridge_hemisphere.py']
        sources+=list(manifest['artifact_sha256'])
        result=dict(status='passed',scope=__doc__,dataset=dataset,checks=checks,
            train=3,test=1,full_training_reset_noise=OmegaConf.to_container(cfg.task.env.initialPoseNoise),
