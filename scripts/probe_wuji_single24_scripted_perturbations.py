"""Measure the single24 scripted contact path on fixed training-grasp perturbations.

This is a feasibility diagnosis, not a learned policy or a new grasp evaluation.
All 100 predeclared states are kept. The same legal four-thumb-joint path drives
hand targets; support fingers retain each perturbed initial preload. Knife pose
and articulation are never commanded. No held-out grasp is loaded or executed.
"""
import argparse
import hashlib
import json
...
def main():
    ...
    chosen = [row for row in candidates if row['dataset_row'] == 24]
    assert len(chosen) == 1 and chosen[0]['continuous']['passed']
    # Use the exact same states as the fixed-time learned CP100 evaluation.
    queue = json.loads((root / 'runs/wuji-goal/audit-queue-wuji_single24_scratch_timed5_seed27_v1.json').read_text())
    first = next(job for job in queue if '-perturb100-timed5seconds' in job['name'])
    fixed_states = root / first['args'][first['args'].index('--initial-states') + 1]
    states = np.load(fixed_states)
    assert states.shape == (100, 75)
    assert hashlib.sha256(fixed_states.read_bytes()).hexdigest() == '2e1ab2d24006d0c358af77ae5d78ab92469e47cbc36f44f304f8a03ba413b3fc'
    paths = np.repeat(np.asarray([chosen[0]['continuous']['thumb_path_rad']], dtype=np.float32), 100, axis=0)
    assert paths.shape == (100, 81, 4)
    assert np.allclose(paths[0, 0], np.load(state_file)[24, 16:20], rtol=0, atol=1e-7)
    ...
    try:
        ...
        report.update(scope=__doc__, source_training_rows=[24] * 100,
                      unattempted_training_rows=[row['dataset_row'] for row in candidates if row['dataset_row'] != 24],
                      ...
                      initial_states_sha256=hashlib.sha256(fixed_states.read_bytes()).hexdigest(),
                      nominal_training_grasps=1, perturbation_trials=100,
