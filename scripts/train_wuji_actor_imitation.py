"""Student-driven recurrent action imitation with a frozen privileged teacher.
...
def main():
    ...
    parser.add_argument('--scope', choices=['head', 'actor'], required=True)
    ...
    allowed = ('a2c_network.mu.',) if args.scope == 'head' else (
    ...
    def save(update):
        ...
        checkpoint = dict(model={k:v.detach().cpu().clone() for k,v in model.state_dict().items()},
            ...
            imitation=dict(scope=args.scope, teacher_sha256=TEACHER_SHA, frozen_student_sha256=STUDENT_SHA,
    ...
    try:
        ...
        for update in range(1, args.updates+1):
            ...
            def forward(ids):
                ...
                for index in range(32):
                    ...
                    result=model.a2c_network(dict(obs=model.norm_obs(raw[ids,index]),
        ...
        report=dict(status='passed',finished=now(),scope=args.scope,updates=args.updates,checks=checks,
            teacher_unchanged=True,student_encoder_unchanged=True,observation_normalizer_unchanged=True,
