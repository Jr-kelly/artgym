"""Collect a second command-clock distribution for a matched state-estimation data diagnostic.
...
def main():
    ...
    manifest=dict(status='passed',seconds=args.seconds,checks=checks,source_teacher_sha256=hashlib.sha256((ROOT/TEACHER).read_bytes()).hexdigest(),
        dataset_sha256=hashlib.sha256(dataset.read_bytes()).hexdigest(),initial_rows=selected.tolist(),
        split='Each training grasp perturbation rows0:20 training,20:30validation; every4thframe ofone20s teacher trajectory; command clock recorded separately in seconds. Reused development states, no independent generalization.',
