"""Frozen-teacher sensor-accuracy and delay interventions with coherent kinematics.
...
def main():
    ...
    def make_player(cfg,path):
        ...
        # Independent RNG: sensor interventions do not consume policy/physics RNG.
        generator=torch.Generator(device=player.device);generator.manual_seed(20261070)
        axis=torch.randn((env.num_envs,3),device=player.device,generator=generator)
        axis=axis/torch.linalg.vector_norm(axis,dim=-1,keepdim=True)
        signs=torch.where(torch.rand((env.num_envs,),device=player.device,generator=generator)<.5,-1.,1.)
        lag=int(args.mode[-1]) if args.mode.startswith('delay') else 0
        delayed=deque(maxlen=lag+1)
        ...
        def observed(obs,is_deterministic=False,**kwargs):
            ...
            delayed.append(true.clone())
            if lag:
                # Warm start holds the first measured state until a lagged state exists.
                modified=delayed[0].clone()
            if args.mode.startswith('position_bias'):
                distance=.0005 if args.mode=='position_bias_0p5mm' else .002
                modified[:,:3]+=axis*distance
            if args.mode.startswith('rotation_bias'):
                angle=(.5 if args.mode=='rotation_bias_0p5deg' else 2.)*np.pi/180
                rotation=torch.cat([axis*np.sin(angle/2),torch.full((env.num_envs,1),float(np.cos(angle/2)),device=player.device)],-1)
                modified[:,3:7]=quat_mul(rotation,modified[:,3:7])
            if args.mode.startswith('slider_bias'):
                distance=.00025 if args.mode=='slider_bias_0p25mm' else .001
                modified[:,19]+=signs*distance
            if args.mode!='full':
                # Keep body and moving-link pose consistent with one prismatic joint.
            ...
            with torch.no_grad():
                ...
                if lag and len(delayed)==1:assert torch.equal(modified[:,:7],true[:,:7])
    ...
    result=dict(status='passed',mode=args.mode,runtime=args.runtime_check,checks=checks,
        ...
        sensor_protocol='Fixed per-environment isotropic direction for body translation/world rotation, fixed random sign for slider bias; shared independent seed20261070. Delays1/3/6 at30Hz; initial measurement held during warm start. Live state remains privileged. Full actor/critic/control weights frozen.',scope=__doc__,source_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest())
    ...
    report.update(policy_kind='privileged_teacher_sensor_accuracy_intervention',input_intervention=result)
