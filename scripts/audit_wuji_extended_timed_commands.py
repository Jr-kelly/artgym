"""Extended fixed-time stress evaluation; arrival never triggers a command change.

Separate entry point preserves the original 20-second evaluator byte-for-byte.
Supports a fresh 60-second horizon and an unseen 3-second command duration.
...
def main():
    ...
    parser.add_argument('--stage-seconds', type=float, choices=[2., 3., 5.], required=True)
    parser.add_argument('--total-seconds', type=int, choices=[20, 60], default=60)
    parser.add_argument('--seed', type=int, default=1616)
    args = parser.parse_args()
    total_steps = args.total_seconds * 30
    if total_steps % round(2 * args.stage_seconds * 30):
        raise ValueError('The duration must contain complete command pairs')
    ...
    # and declared episode termination stay active. Success is scored from trace.
    ...
    assert abs(dt - 1/30) < 1e-6
    assert total_steps % (2 * stage_steps) == 0
    frames = []
    ...
    def observe_and_schedule(actions):
        ...
        if len(frames) % stage_steps == 0 and len(frames) < total_steps:
    ...
    try:
        ...
        report = score_timed_trace(trace, stage_steps, 9, total_steps)
        report.update(checkpoint_sha256=hashlib.sha256(args.checkpoint.read_bytes()).hexdigest(),
            ...
            protocol=dict(stage_seconds=args.stage_seconds, stage_steps=stage_steps, control_dt=dt,
                          goal_tolerance_m=.002, hold_steps=9, schedule='alternate open/close', total_seconds=args.total_seconds, expected_steps=total_steps,
        ...
        for seconds in [20, 40, 60]:
            count = seconds * 30
            if count <= total_steps and count % (2 * stage_steps) == 0:
                prefix = {key: value[:count] for key, value in trace.items()}
                metrics = score_timed_trace(prefix, stage_steps, 9, count)
                report['prefix_metrics'][str(seconds)] = {key: value for key, value in metrics.items() if key != 'records'}
