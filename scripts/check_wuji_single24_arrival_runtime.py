"""Check training-only resets and legal actions for the single24 arrival-acquisition case."""
import argparse
import hashlib
import json
...
def main():
    ...
    cfg = configuration('wuji_acquisition_official_arrival_control', 50,
    ...
    reference = configuration('wuji_acquisition_official_timed5', 50,
        ['object=' + name, 'hand=wuji_paper_official_actuator', 'test=False', 'object.reward.GoalDistance2=5.0'],
        train='wujiAcquisitionSAPG', seed=20261027)
    before = OmegaConf.to_container(reference, resolve=True)
    after = OmegaConf.to_container(cfg, resolve=True)
    assert before['task']['env']['commandPeriodSec'] == 5
    before['task']['env']['commandPeriodSec'] = 0.0
    assert before == after, 'Unexpected extra configuration difference in matched acquisition pair'
    (args.output / 'config.yaml').write_text(OmegaConf.to_yaml(cfg, resolve=True))
    original_sample = ArtManip.sample_grasps
    checks = dict(sampled_rows=[0], transitions=0, arrival_switches=0, action_mapping_calls=0)
    ...
    def reward(actions):
        ...
        due = env.goal_achieved_step.bool()
        assert torch.equal((goals != env.goal_obj_dof_pos).any(-1), due)
        checks['arrival_switches'] += int(due.sum())
    ...
    try:
        ...
        assert cfg.task.env.commandPeriodSec == 0 and cfg.object.reward.GoalDistance2 == 5
        ...
        # Zero/small actions must not switch to easy closing without opening.
        assert checks['transitions'] == 10000 and checks['arrival_switches'] == 0
        sources = ['scripts/check_wuji_single24_arrival_runtime.py', 'isaacgymenvs/tasks/artmanip.py',
                   'isaacgymenvs/tasks/wuji_acquisition.py', 'isaacgymenvs/tasks/wuji_timed_acquisition.py',
                   'isaacgymenvs/cfg/task/wuji_acquisition_official_arrival_control.yaml',
