...
def main():
    if (ROOT / 'FINAL.json').exists():
        ...
        progress(final['status'], release=final['release'], files=final['files'])
        return
    env = os.environ.copy()
    ...
    sealed = ROOT / 'delivery-sealed.json'
    if not sealed.exists():
        ...
        with tarfile.open(evidence, 'w:gz') as archive:
            ...
            for receipt in sorted((ROOT / 'upload-receipts').glob('*.tar.zst.json')):
                archive.add(receipt, arcname='upload-receipts/' + receipt.name)
    ...
    '''
        readme = '\n'.join(line[4:] if line.startswith('    ') else line for line in readme.splitlines()) + '\n'
        ...
        atomic(sealed, dict(created=now(), files={p.name: sha(p) for p in delivery.iterdir() if p.is_file()}))
    sealed_files = json.loads(sealed.read_text())['files']
    assert all(sha(delivery / name) == digest for name, digest in sealed_files.items()), 'Frozen delivery changed'
    coverage = json.loads((delivery / 'wuji-full-backup-20260925-coverage.json').read_text())
    readme = (delivery / 'README.md').read_text()
    checks = [(line.split('  ', 1)[0], line.split('  ', 1)[1]) for line in (delivery / 'SHA256SUMS.txt').read_text().splitlines()]
    remote_pending = index['remote_pending']
