...
def main():
    ...
    p.add_argument('--distance',type=float,default=.010);p.add_argument('--stage-prefix',default='held_translation');a=p.parse_args()
    if a.output.exists():raise ValueError('Preserve old geometry')
    if not 0<a.distance<=.010:raise ValueError('Translation screen limited to10mm')
    ...
    for knot in range(1,6):
        relative=r0.copy();relative[:3,3]+=delta*knot/5
        def errors(qv):
        ...
        goal=arm0.copy();goal[:3,3]+=obj[:3,:3]@delta*knot/5;qa,ik=k.solve_near(goal,qa,max_step=.15)
        ...
        if not valid:break
    meta=dict(source=str(a.source),knife_center_in_hand_start_m=actual_center.tolist(),functional_knife_center_in_hand_m=desired_center.tolist(),planned_wrist_translation_in_knife_m=delta.tolist(),rows=rows,
        ...
        scope='Geometry only, <=10mm wrist translation with three opposed digits. Original command-minus-measured offsets retained; no physics or initial state changes.')
