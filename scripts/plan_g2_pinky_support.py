...
def main():
    ...
    if a.surface=='underside':normals[4]=[0,-1,0]
    point=c.contacts(q,relative,normals)[0][4]
    ...
    def solve(target,seed,command=False,free=False):
        def residual(v):
            qs,pt=surface(v)
            return np.r_[(pt-target)*300,[min(s['gap_lower_bound_m']-(-.0011 if command else (.0048 if free else .00005)),0)*600 for s in g.gaps(qs,relative,-.032674588,'pinky')],
    ...
    pre=None;preq=None;prepoint=None
    if a.approach_under:
        prepoint=target.copy();prepoint[1]-=.012;pre=solve(prepoint,q[8:12],free=True);preq=surface(pre.x)[0]
    touch=solve(target,pre.x if pre is not None else q[8:12]);touchq,actual_point=surface(touch.x);goal=target.copy()
    ...
    for piece,(begin,end) in enumerate(pieces):
        for alpha in np.linspace(0,1,31):
            test=begin*(1-alpha)+end*alpha;selfgap=min(v['gap_lower_bound_m'] for v in full.self_gaps(test,'pinky'))
            sweep.append(dict(segment=piece,alpha=float(alpha),knife_gap_m=full.minimum_gap(test,relative,-.032674588,'pinky'),self_gap_m=selfgap))
    ...
    if preq is not None:valid=valid and full.minimum_gap(preq,relative,-.032674588,'pinky')>=.0041 and np.linalg.norm(surface(pre.x)[1]-prepoint)<.001
    close_sweep=[]
    ...
    stages=[]
    if preq is not None:
        stages += [dict(name='pinky_under_clearance',kind='move',moving_indices=list(range(8,12)),target=pre.x.tolist(),seconds=max(2.,1.875*np.max(abs(pre.x-q[8:12]))),require_thumb_gap_m=.0041),
            dict(name='pinky_under_clear_hold',kind='hold',seconds=1.,require_contacts=[0,2,3] if a.held_thumb else [1,2,3],require_no_contact=4,require_thumb_gap_m=.0041)]
    stages += [dict(name='pinky_side_touch',kind='move',moving_indices=list(range(8,12)),target=touch.x.tolist(),seconds=2.,require_thumb_gap_m=.0041),
