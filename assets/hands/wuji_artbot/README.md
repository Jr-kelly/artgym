...
Extracted from the supplied ArtBot asset `G2_crsB_wuji/robot.usd` on 2026-09-20.
The original whole-robot USD is not included in this branch.
...
The base hand config disables self-collision, matching the source articulation.
The default knife demo overrides the shape filters to enable cross-finger and
distal-to-palm collisions.
...
```bash
python scripts/import_wuji_artbot.py \
  --source /path/to/G2_crsB_wuji/robot.usd
